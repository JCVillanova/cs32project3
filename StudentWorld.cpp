...
: GameWorld(assetPath), actorList(VIEW_WIDTH, vector<list<Actor*>>(VIEW_HEIGHT))
{
}
...
{
	// Check level
	string currentLevel = "level0" + to_string(getLevel()) + ".txt";
	Level lev(assetPath());
...
list<Actor*>::iterator StudentWorld::createIterator(int x, int y)
{
...
void StudentWorld::removeActor(list<Actor*>::iterator it, int x, int y)
{
	delete *it;
	it = actorList[x][y].erase(it);
}

list<Actor*> StudentWorld::getActorsInCell(int x, int y) const
{
